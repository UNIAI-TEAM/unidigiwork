...
SELECT * FROM public.provision_tenant(
  'itest_go4_A', 'itest-go4-a-'||substr(md5(random()::text),1,10), :OWNER_A::uuid, 'GO4-A');

SELECT pg_temp.act(:OWNER_B::uuid);
INSERT INTO _t
SELECT * FROM public.provision_tenant(
  'itest_go4_B', 'itest-go4-b-'||substr(md5(random()::text),1,10), :OWNER_B::uuid, 'GO4-B');
...
BEGIN
  ...
  VALUES (_tid, _ws, 'Board Report', 'REPORT', 'IN_REVIEW', 1, :OWNER_A::uuid, :OWNER_A::uuid)
  ...
  VALUES (_tid, _wp, :OWNER_A::uuid, :OWNER_A::uuid, 'APPROVED', now());
...
BEGIN
  ...
  INSERT INTO public.work_products (tenant_id, workspace_id, title, business_type, status, current_version, created_by, owner_id)
  VALUES (_tid, _ws, 'Financial Spreadsheet', 'ANALYSIS', 'IN_REVIEW', 1, :OWNER_A::uuid, :OWNER_A::uuid)
  RETURNING id INTO _wp;
...
BEGIN
  ...
  VALUES (_wp, _tid, 2, 'snap', :OWNER_A::uuid);
...
BEGIN
  ...
  VALUES (_tid, _ws, 'B secret WP', 'MEMO', 'DRAFT', 1, :OWNER_B::uuid)
  RETURNING id INTO _wp;
