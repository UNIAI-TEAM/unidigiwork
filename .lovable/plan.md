...
### Dữ liệu
- `work_products`: tiêu đề, mô tả, loại nghiệp vụ (đề xuất, báo cáo, phân tích, hợp đồng, kế hoạch, thuyết trình...), định dạng tệp (native/docx/xlsx/pptx/pdf/markdown) tách riêng, trạng thái (nháp → đang xem xét → yêu cầu sửa → đã duyệt → hoàn tất → lưu trữ), chủ sở hữu, tham chiếu lưu trữ, phiên bản hiện tại, người/AI tạo, tổ chức + dự án.
- `work_product_versions`: số phiên bản, ảnh chụp nội dung/tham chiếu tệp, tóm tắt thay đổi, người/AI tạo, ngữ cảnh AI đã dùng (nguồn gốc thay đổi).
...
- Liên kết công việc dùng **Work Graph sẵn có**: bổ sung loại thực thể `WORK_PRODUCT` và các quy tắc quan hệ (thuộc dự án, tạo từ cuộc họp, tham chiếu quyết định/công việc/tài liệu).
...
- Thanh công cụ AI khi bôi đen: Hỏi AI, Cải thiện, Rút gọn, Mở rộng, Viết lại, Dịch — luôn hiện bản xem trước với Chấp nhận / Từ chối / Tạo lại; chỉ ghi khi người dùng chấp nhận, và ghi lại nguồn ngữ cảnh.
