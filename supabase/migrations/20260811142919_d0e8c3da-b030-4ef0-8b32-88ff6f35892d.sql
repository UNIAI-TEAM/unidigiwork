CREATE OR REPLACE FUNCTION public.log_meeting_host_action(
  _meeting_id uuid,
  _action text,
  _outcome text,
...
BEGIN
  ...
  IF _action NOT IN ('start','end') THEN RAISE EXCEPTION 'INVALID_ACTION' USING ERRCODE='22023'; END IF;
...
REVOKE ALL ON FUNCTION public.log_meeting_host_action(uuid, text, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.log_meeting_host_action(uuid, text, text, text, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.log_meeting_host_action(uuid, text, text, text, text, text) TO service_role;