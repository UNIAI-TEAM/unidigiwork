...
-- Redeclare with DEFAULT NULL on optional params so generated TS types mark them optional.
-- Bodies unchanged; only signatures gain DEFAULTs. Must DROP + CREATE because Postgres
-- doesn't allow changing default of existing function signature via CREATE OR REPLACE
-- when arg-list stays the same (it does actually allow it). Use CREATE OR REPLACE.

CREATE OR REPLACE FUNCTION public.create_task(
  _workspace_id uuid, _title text,
  _description text DEFAULT NULL,
  _priority task_priority DEFAULT 'normal',
...
CREATE OR REPLACE FUNCTION public.update_task(
  ...
  _priority task_priority DEFAULT NULL, _due_at timestamptz DEFAULT NULL,
...
BEGIN
  ...
  UPDATE public.tasks SET
    title = COALESCE(_title, title), description = COALESCE(_description, description),
    priority = COALESCE(_priority, priority), due_at = COALESCE(_due_at, due_at),
    updated_by = _actor
...
CREATE OR REPLACE FUNCTION public.transition_task(
  _task_id uuid, _to_status task_status,
  _expected_row_version bigint DEFAULT NULL,
  _idempotency_key text DEFAULT NULL, _correlation_id text DEFAULT NULL
) RETURNS public.tasks LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
CREATE OR REPLACE FUNCTION public.assign_task(
  _task_id uuid, _assignee_id uuid,
  _role text DEFAULT 'assignee',
  _idempotency_key text DEFAULT NULL, _correlation_id text DEFAULT NULL
) RETURNS public.tasks LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _actor uuid := auth.uid(); _task public.tasks;
BEGIN
...
CREATE OR REPLACE FUNCTION public.create_document(
  _workspace_id uuid, _title text,
  _folder text DEFAULT 'My Documents', _tags text[] DEFAULT '{}'::text[],
  _storage_ref jsonb DEFAULT NULL, _mime_type text DEFAULT NULL, _size_bytes bigint DEFAULT 0,
  _idempotency_key text DEFAULT NULL, _correlation_id text DEFAULT NULL
...
CREATE OR REPLACE FUNCTION public.update_document(
  _document_id uuid,
  _title text DEFAULT NULL, _folder text DEFAULT NULL, _tags text[] DEFAULT NULL,
  _expected_row_version bigint DEFAULT NULL,
  _idempotency_key text DEFAULT NULL, _correlation_id text DEFAULT NULL
) RETURNS public.documents LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
BEGIN
  ...
  UPDATE public.documents SET
    title = COALESCE(_title, title), folder = COALESCE(_folder, folder),
    tags = COALESCE(_tags, tags), updated_by = _actor
...
CREATE OR REPLACE FUNCTION public.upload_document_version(
  _document_id uuid, _storage_ref jsonb,
  _mime_type text DEFAULT NULL, _size_bytes bigint DEFAULT 0, _comment text DEFAULT NULL,
  _idempotency_key text DEFAULT NULL, _correlation_id text DEFAULT NULL
) RETURNS public.document_versions LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
CREATE OR REPLACE FUNCTION public.share_document(
  ...
  _idempotency_key text DEFAULT NULL, _correlation_id text DEFAULT NULL
) RETURNS public.document_permissions LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
CREATE OR REPLACE FUNCTION public.schedule_meeting(
  ...
  _agenda text DEFAULT NULL, _timezone text DEFAULT 'UTC',
...
CREATE OR REPLACE FUNCTION public.update_meeting(
  _meeting_id uuid,
  _title text DEFAULT NULL, _agenda text DEFAULT NULL,
  _start_at timestamptz DEFAULT NULL, _end_at timestamptz DEFAULT NULL,
  _timezone text DEFAULT NULL, _location text DEFAULT NULL,
  _expected_row_version bigint DEFAULT NULL,
  _idempotency_key text DEFAULT NULL, _correlation_id text DEFAULT NULL
) RETURNS public.meetings LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
CREATE OR REPLACE FUNCTION public.cancel_meeting(
  _meeting_id uuid,
  _reason text DEFAULT NULL, _expected_row_version bigint DEFAULT NULL,
  _idempotency_key text DEFAULT NULL, _correlation_id text DEFAULT NULL
) RETURNS public.meetings LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _actor uuid := auth.uid(); _m public.meetings;
BEGIN
...
CREATE OR REPLACE FUNCTION public.set_meeting_rsvp(
  ...
  _idempotency_key text DEFAULT NULL, _correlation_id text DEFAULT NULL
) RETURNS public.meeting_participants LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
CREATE OR REPLACE FUNCTION public.create_workflow(
  _workspace_id uuid, _name text, _definition jsonb,
  _description text DEFAULT NULL,
  _idempotency_key text DEFAULT NULL, _correlation_id text DEFAULT NULL
) RETURNS public.workflows LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
DECLARE _actor uuid := auth.uid(); _run public.workflow_runs; _step public.workflow_steps;
BEGIN
  IF _actor IS NULL THEN RAISE EXCEPTION 'AUTHENTICATION_REQUIRED' USING ERRCODE='42501'; END IF;
