-- Concurrent same-key protection at DB level for audit-based idempotency ledger.
CREATE UNIQUE INDEX IF NOT EXISTS audit_events_idem_event_uniq
  ON public.audit_events (event_type, idempotency_key)
...
BEGIN
  ...
  -- Payload fingerprint: any meaningful change with same key => IDEMPOTENCY_CONFLICT.
  _fp := encode(digest(
    coalesce(_name,'') || '|' || _norm_slug || '|' || _owner_id::text || '|' ||
    coalesce(_default_workspace_name,''), 'sha256'), 'hex');

  IF _idempotency_key IS NOT NULL THEN
    SELECT (payload->>'tenant_id')::uuid, payload->>'fingerprint', payload
      INTO _existing_tenant, _existing_fp, _existing_payload
  ...
  EXCEPTION WHEN unique_violation THEN
    -- Concurrent same-key race: read winner's result and return it.
    SELECT (payload->>'tenant_id')::uuid INTO _existing_tenant
      FROM public.audit_events
      WHERE event_type = 'tenant.provisioned' AND idempotency_key = _idempotency_key
      LIMIT 1;
    IF _existing_tenant IS NULL THEN
      RAISE;
    END IF;
    -- Roll back this attempt's inserts by raising a sentinel; the caller sees
    -- the winner's data via idempotent replay path on next call. Since we're
    -- inside a single tx, raise to abort and let PostgREST retry.
