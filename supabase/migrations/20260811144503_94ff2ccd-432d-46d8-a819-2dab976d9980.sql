...
BEGIN
  ...
  IF _action NOT IN ('start','end','cancel','transfer_host') THEN RAISE EXCEPTION 'INVALID_ACTION' USING ERRCODE='22023'; END IF;
...
CREATE OR REPLACE FUNCTION public.transfer_meeting_host(_meeting_id uuid, _new_host_user_id uuid, _idempotency_key text DEFAULT NULL::text, _correlation_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE _actor uuid := auth.uid(); _m public.meetings; _actor_role text; _target_role text;
BEGIN
  IF _actor IS NULL THEN RAISE EXCEPTION 'AUTHENTICATION_REQUIRED' USING ERRCODE='28000'; END IF;
  ...
  SELECT mp.role INTO _actor_role FROM public.meeting_participants mp
    WHERE mp.meeting_id = _meeting_id AND mp.user_id = _actor;
  IF _actor_role IS DISTINCT FROM 'host' THEN
    RAISE EXCEPTION 'MEETING_ACCESS_DENIED' USING ERRCODE='42501';
  END IF;

  IF _new_host_user_id = _actor THEN
    RAISE EXCEPTION 'ALREADY_HOST' USING ERRCODE='22023';
  END IF;

  SELECT mp.role INTO _target_role FROM public.meeting_participants mp
    WHERE mp.meeting_id = _meeting_id AND mp.user_id = _new_host_user_id;
  IF _target_role IS NULL THEN
    RAISE EXCEPTION 'PARTICIPANT_NOT_FOUND' USING ERRCODE='02000';
  END IF;
  ...
  UPDATE public.meeting_participants SET role = 'participant', updated_at = now()
    WHERE meeting_id = _meeting_id AND user_id = _actor;
  UPDATE public.meeting_participants SET role = 'host', updated_at = now()
    WHERE meeting_id = _meeting_id AND user_id = _new_host_user_id;

  UPDATE public.meetings SET updated_by = _actor, updated_at = now(), row_version = row_version + 1
    WHERE id = _meeting_id;

  RETURN jsonb_build_object('meeting_id', _meeting_id, 'previous_host', _actor, 'new_host', _new_host_user_id);
END $function$;

REVOKE ALL ON FUNCTION public.transfer_meeting_host(uuid, uuid, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.transfer_meeting_host(uuid, uuid, text, text) TO authenticated;