CREATE OR REPLACE FUNCTION public.accept_ai_task_execution(_execution_id uuid, _complete_task boolean DEFAULT true, _idempotency_key text DEFAULT NULL, _correlation_id uuid DEFAULT NULL)
...
BEGIN
  ...
  IF _complete_task AND _task.status <> 'done' THEN
    -- Chuyển tiếp hợp lệ: todo/blocked phải qua in_progress trước khi hoàn thành.
