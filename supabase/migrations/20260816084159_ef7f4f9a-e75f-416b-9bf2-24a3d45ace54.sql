-- Trigram indexes for artifact search
CREATE INDEX IF NOT EXISTS idx_meeting_artifacts_title_norm_trgm
  ON public.meeting_artifacts USING gin (public.search_norm(title) gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_meeting_artifacts_detail_norm_trgm
  ON public.meeting_artifacts USING gin (public.search_norm(detail) gin_trgm_ops);

DROP FUNCTION IF EXISTS public.search_universal(text, uuid, text[], uuid, integer, integer);

CREATE OR REPLACE FUNCTION public.search_universal(
  _q text,
  _tenant_id uuid,
  _entity_types text[] DEFAULT NULL::text[],
...
BEGIN
  ...
  WITH raw AS (
    ...
    (SELECT 'MEETING', m.id, m.title,
     ...
     LIMIT per)
    UNION ALL
    -- MEETING_ARTIFACT (summary / key points / decisions / action items / risks / follow-up)
