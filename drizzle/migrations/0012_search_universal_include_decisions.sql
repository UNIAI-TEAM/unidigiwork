...
BEGIN
  ...
  WITH raw AS (
    ...
    (SELECT 'MEETING_ARTIFACT', a.id,
     ...
     LIMIT per)
    UNION ALL
    (SELECT 'DECISION', dc.id, left(dc.title, 200),
            'Quyết định · ' || CASE dc.status
              WHEN 'CONFIRMED' THEN 'Đã xác nhận'
              WHEN 'CANDIDATE' THEN 'Chờ xác nhận'
              WHEN 'REJECTED' THEN 'Đã từ chối'
              ELSE 'Đã bị thay thế' END,
            coalesce(left(coalesce(dc.detail, dc.title), 200), ''),
            dc.workspace_id, dc.updated_at, public.search_norm(dc.title),
            NULL::text, NULL::uuid, NULL::text
     FROM public.decisions dc
     WHERE dc.tenant_id = _tenant_id
       AND dc.status <> 'REJECTED'
       AND (_workspace_id IS NULL OR dc.workspace_id = _workspace_id OR dc.workspace_id IS NULL)
       AND (public.search_norm(dc.title) LIKE '%' || nq || '%'
            OR public.search_norm(dc.title) % nq
            OR public.search_norm(coalesce(dc.detail, '')) LIKE '%' || nq || '%')
     LIMIT per)
    UNION ALL
    (SELECT 'DOCUMENT', d.id, d.title,
     ...
     LIMIT per)
    UNION ALL
    (SELECT 'WORK_PRODUCT', p.id, p.title,
            'Kết quả công việc · ' || p.business_type || ' · ' || p.status,
            coalesce(left(coalesce(nullif(p.description, ''), p.content), 160), ''),
            p.workspace_id, p.updated_at, public.search_norm(p.title),
            NULL::text, NULL::uuid, NULL::text
     FROM public.work_products p
     WHERE p.deleted_at IS NULL AND p.tenant_id = _tenant_id
       AND (_workspace_id IS NULL OR p.workspace_id = _workspace_id)
       AND (public.search_norm(p.title) LIKE '%' || nq || '%'
            OR public.search_norm(p.title) % nq
            OR public.search_norm(coalesce(p.content, '')) LIKE '%' || nq || '%')
     LIMIT per)
    UNION ALL
  ...
  scored AS (
    SELECT r.*,
           ...
           + CASE WHEN r.etype = 'PROJECT' THEN 6 WHEN r.etype = 'TASK' THEN 3
                  WHEN r.etype = 'WORK_PRODUCT' THEN 3
                  WHEN r.etype = 'DECISION' THEN 4
