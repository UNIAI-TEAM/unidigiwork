DROP FUNCTION IF EXISTS public.extract_decisions_from_meeting(uuid);

CREATE OR REPLACE FUNCTION public.extract_decisions_from_meeting(_meeting_id uuid)
 RETURNS TABLE(created integer, skipped integer, total integer, decision_ids uuid[])
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ids of all decisions extracted from this meeting (existing + new)
  SELECT COALESCE(array_agg(d.id), '{}'::uuid[]) INTO ids
    FROM public.decisions d
   WHERE d.tenant_id = m.tenant_id
     AND d.origin = 'MEETING'
     AND d.source_id = m.id;

  RETURN QUERY SELECT c, t - c, t, ids;
END $function$;

REVOKE ALL ON FUNCTION public.extract_decisions_from_meeting(uuid) FROM public;
GRANT EXECUTE ON FUNCTION public.extract_decisions_from_meeting(uuid) TO authenticated, service_role;