DROP FUNCTION IF EXISTS public.persist_chat_executive_brief(uuid,text,text,uuid,jsonb,text,text);

CREATE OR REPLACE FUNCTION public.persist_chat_executive_brief(
  _assistant_message_id uuid,
  _title text,
  _report_content text DEFAULT NULL,
...
DECLARE
  _actor uuid := auth.uid(); _message public.ai_messages; _conversation public.ai_conversations;
  _work_product public.work_products; _source jsonb; _source_type text; _source_id uuid;
  _existing_id uuid; _content text; _provenance jsonb;
BEGIN
  ...
  END IF;
  _content:=coalesce(nullif(btrim(_report_content),''),_message.content);
  _provenance:=jsonb_build_array(jsonb_build_object('entityType','AI_MESSAGE','entityId',_message.id,'conversationId',_conversation.id,'executiveBrief',_message.content,'reportMetadata',coalesce(_report_metadata,'{}'::jsonb),'capturedAt',now()));
  INSERT INTO public.work_products(tenant_id,workspace_id,title,description,business_type,status,content,owner_id,created_by,ai_generated,current_version,primary_context_type,primary_context_id,tags)
  VALUES(_message.tenant_id,_conversation.workspace_id,left(coalesce(nullif(btrim(_title),''),'Executive Brief'),300),left('Báo cáo đầy đủ từ Executive Brief trong hội thoại '||_conversation.title,2000),'REPORT','DRAFT',_content,_actor,_actor,true,1,_root_type,_root_id,ARRAY['EXECUTIVE_BRIEF','AI_CHAT','AUTO_REPORT','FULL_REPORT']::text[])
  RETURNING * INTO _work_product;
  INSERT INTO public.work_product_versions(tenant_id,work_product_id,version,title,content,summary,provenance,ai_generated,author_id)
  VALUES(_message.tenant_id,_work_product.id,1,_work_product.title,_content,'Báo cáo đầy đủ được tạo tự động từ Executive Brief và tiến độ Work Graph.',_provenance,true,_actor);
  ...
  IF jsonb_typeof(coalesce(_sources,'[]'::jsonb))='array' THEN
    FOR _source IN SELECT value FROM jsonb_array_elements(coalesce(_sources,'[]'::jsonb)) LOOP
      ...
      IF _source_type IN ('TASK','DOCUMENT','MEETING','MEETING_ARTIFACT') AND _source_id IS NOT NULL AND public.can_view_work_entity(_source_type,_source_id) THEN
    ...
    END LOOP;
  END IF;
  UPDATE public.ai_messages SET metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('workProductId',_work_product.id,'workProductHref','/work-products/'||_work_product.id::text,'workProductStatus','CREATED','workProductKind','REPORT','workProductVersion',1,'reportFormat','FULL_REPORT') WHERE id=_message.id;
  PERFORM public._emit_outbox_event(_message.tenant_id,'work_product.created_from_chat','work_product',_work_product.id::text,jsonb_build_object('work_product_id',_work_product.id,'conversation_id',_conversation.id,'assistant_message_id',_message.id,'root_type',_root_type,'root_id',_root_id,'business_type','REPORT','version',1,'report_format','FULL_REPORT'),_idempotency_key,_correlation_id);
  RETURN jsonb_build_object('id',_work_product.id,'created',true,'href','/work-products/'||_work_product.id::text,'kind','REPORT','version',1,'format','FULL_REPORT');
END;
$$;
REVOKE ALL ON FUNCTION public.persist_chat_executive_brief(uuid,text,text,jsonb,text,uuid,jsonb,text,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.persist_chat_executive_brief(uuid,text,text,jsonb,text,uuid,jsonb,text,text) TO authenticated, service_role;