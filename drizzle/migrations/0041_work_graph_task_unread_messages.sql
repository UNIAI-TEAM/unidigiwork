CREATE OR REPLACE FUNCTION public.mark_task_message_notifications_read(_task_id uuid)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET is_read = true,
      read_at = now(),
      updated_at = now()
  WHERE n.user_id = _actor
    AND n.tenant_id = _tenant_id
    AND n.type = 'task.task.message_sent'
    AND n.is_read = false
    AND n.meta->>'aggregate_id' = _task_id::text;
...
REVOKE ALL ON FUNCTION public.mark_task_message_notifications_read(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.mark_task_message_notifications_read(uuid) TO authenticated, service_role;

CREATE INDEX IF NOT EXISTS notifications_task_message_unread_idx
ON public.notifications (user_id, ((meta->>'aggregate_id')), created_at DESC)
WHERE is_read = false AND type = 'task.task.message_sent';
...
enriched AS (
 SELECT p.*,
   (SELECT count(*)::integer FROM public.work_edges e WHERE e.tenant_id=_tenant_id AND (e.source_node_id=p.node_id OR e.target_node_id=p.node_id)) links,
   CASE WHEN p.entity_type='TASK' THEN COALESCE(human_stats.message_count,0)+COALESCE(ai_stats.message_count,0) ELSE 0 END interaction_count,
   CASE WHEN p.entity_type='TASK' THEN GREATEST(human_stats.last_message_at,ai_stats.last_message_at) ELSE NULL::timestamptz END last_interaction_at,
   CASE WHEN p.entity_type='TASK' THEN COALESCE(unread_stats.unread_count,0) ELSE 0 END unread_message_count
 FROM page_rows p
 LEFT JOIN LATERAL (
   SELECT count(*)::integer message_count,max(tc.created_at) last_message_at
   FROM public.task_comments tc
   WHERE p.entity_type='TASK' AND tc.tenant_id=_tenant_id AND tc.task_id=p.entity_id AND tc.deleted_at IS NULL
 ) human_stats ON p.entity_type='TASK'
 LEFT JOIN LATERAL (
   SELECT count(*)::integer message_count,max(am.created_at) last_message_at
   FROM public.ai_messages am
   WHERE p.entity_type='TASK' AND am.tenant_id=_tenant_id
     AND am.conversation_id IN (
       SELECT linked.conversation_id FROM (
         SELECT lm.conversation_id
         FROM public.ai_messages lm
         WHERE lm.tenant_id=_tenant_id AND (
           lm.metadata @> jsonb_build_object('contextEntities',jsonb_build_array(jsonb_build_object('type','TASK','id',p.entity_id::text)))
           OR lm.metadata @> jsonb_build_object('sources',jsonb_build_array(jsonb_build_object('entityType','TASK','entityId',p.entity_id::text)))
         )
         UNION
         SELECT ap.conversation_id
         FROM public.ai_action_proposals ap
         WHERE ap.tenant_id=_tenant_id AND ap.conversation_id IS NOT NULL AND ap.status='SUCCEEDED' AND ap.action_type='CREATE_TASK'
           AND ap.result @> jsonb_build_object('entityType','TASK','entityId',p.entity_id::text)
       ) linked
     )
 ) ai_stats ON p.entity_type='TASK'
 LEFT JOIN LATERAL (
   SELECT count(*)::integer unread_count
   FROM public.notifications nn
   WHERE p.entity_type='TASK'
     AND nn.user_id=auth.uid()
     AND nn.tenant_id=_tenant_id
     AND nn.type='task.task.message_sent'
     AND nn.is_read=false
     AND nn.meta->>'aggregate_id'=p.entity_id::text
 ) unread_stats ON p.entity_type='TASK'
)
...
GRANT EXECUTE ON FUNCTION public.list_work_graph_board_page(uuid,text,text,uuid,boolean,text,uuid,integer,integer) TO authenticated, service_role;