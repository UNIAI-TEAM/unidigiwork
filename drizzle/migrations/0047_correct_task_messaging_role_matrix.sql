...
AS $$
  SELECT
    (tm.role IN ('member','guest') AND EXISTS (
      SELECT 1 FROM public.task_assignees ta
      WHERE ta.task_id=t.id AND ta.tenant_id=t.tenant_id AND ta.user_id=auth.uid()
    )),
    (tm.role IN ('tenant_owner','tenant_admin','manager')),
    (tm.role IN ('tenant_owner','tenant_admin','manager'))
  FROM public.tasks t
  JOIN public.tenant_members tm
    ON tm.tenant_id=t.tenant_id AND tm.user_id=auth.uid() AND tm.status='active'
...
AS $$
  ...
  JOIN public.tenant_members caller
    ON caller.tenant_id=t.tenant_id AND caller.user_id=auth.uid() AND caller.status='active'
   AND caller.role IN ('tenant_owner','tenant_admin','manager')
  JOIN public.tenant_members superior
    ON superior.tenant_id=t.tenant_id AND superior.status='active'
   AND superior.role IN ('tenant_owner','tenant_admin','manager') AND superior.user_id<>auth.uid()
...
CREATE OR REPLACE FUNCTION public.comment_task_to_assignee(
  _task_id uuid,_recipient_id uuid,_body text,_idempotency_key text,_correlation_id text,_source text
) RETURNS public.task_comments
LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE
  _actor uuid:=auth.uid(); _task public.tasks; _comment public.task_comments; _existing uuid;
  _src text:=upper(coalesce(_source,'TASK_CHAT')); _actor_role public.tenant_role;
BEGIN
  ...
  SELECT tm.role INTO _actor_role FROM public.tenant_members tm
  ...
  IF _src='TASK_CHAT' THEN
    IF _actor_role NOT IN ('member','guest') OR NOT EXISTS (
      SELECT 1 FROM public.task_assignees ta
      WHERE ta.task_id=_task_id AND ta.tenant_id=_task.tenant_id AND ta.user_id=_actor
    ) THEN RAISE EXCEPTION 'TASK_MESSAGE_ROLE_FORBIDDEN' USING ERRCODE='42501'; END IF;
    IF NOT EXISTS (
      SELECT 1 FROM public.task_assignees ta
  ...
  ELSE
    IF _actor_role NOT IN ('tenant_owner','tenant_admin','manager') THEN
      RAISE EXCEPTION 'TASK_MESSAGE_ROLE_FORBIDDEN' USING ERRCODE='42501';
    END IF;
    IF _recipient_id=_actor OR NOT EXISTS (
  ...
  SELECT nullif(payload->>'comment_id','')::uuid INTO _existing FROM public.outbox_events
  ...
  PERFORM public._emit_outbox_event(
    ...
    jsonb_build_object('task_id',_task.id,'comment_id',_comment.id,'author_id',_actor,
      'recipient_user_ids',jsonb_build_array(_recipient_id::text),
      'title',CASE WHEN _src='PRIVATE_SUPERIOR' THEN 'Tin nhắn riêng từ cấp trên' ELSE 'Tin nhắn mới trong công việc' END,
...
DROP POLICY IF EXISTS task_comments_tenant_select ON public.task_comments;
CREATE POLICY task_comments_tenant_select ON public.task_comments FOR SELECT TO authenticated
USING (
  public.is_tenant_member(tenant_id) AND deleted_at IS NULL AND (
    coalesce(metadata->>'source','') <> 'PRIVATE_SUPERIOR'
    OR author_id=auth.uid()
    OR metadata->>'recipient_id'=auth.uid()::text
  )
);