CREATE OR REPLACE FUNCTION public.list_task_following_page(
  _tenant_id uuid,
...
BEGIN
  ...
  WITH visible_tasks AS (
    SELECT
      ...
      EXISTS (
        SELECT 1
        FROM public.task_followers mine
        WHERE mine.task_id = t.id
          AND mine.user_id = auth.uid()
      ) AS following,
      (
        SELECT count(*)
        FROM public.task_followers followers
        WHERE followers.task_id = t.id
      ) AS follower_count
    FROM public.tasks t
    ...
    WHERE t.tenant_id = _tenant_id
      ...
      AND (
        ...
        OR t.status::text = ANY(_statuses)
      )
      AND (
    ...
    WHERE _following IS NULL OR v.following = _following
  )
  SELECT
    f.id,
    f.title,
...
REVOKE ALL ON FUNCTION public.list_task_following_page(uuid, boolean, text[], text, integer, integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.list_task_following_page(uuid, boolean, text[], text, integer, integer) TO authenticated, service_role;