...
export const listWorkDeliverables = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [ws, profiles] = await Promise.all([
      ...
      ownerIds.length
        ? context.supabase.from("profiles").select("id, display_name, email").in("id", ownerIds)
        : Promise.resolve({ data: [] }),
    ]);
    const wmap = new Map(((ws.data ?? []) as any[]).map((w) => [w.id, w.name]));
    const pmap = new Map(
      ((profiles.data ?? []) as any[]).map((p) => [p.id, p.display_name ?? p.email]),
    );
  ...
  .handler(async ({ data, context }) => {
    ...
    const [versions, comments, reviews, artifacts, workspace] = await Promise.all([
      ...
      context.supabase
        ...
        .select(
          "id, reviewer_id, requested_by, status, due_at, decision_note, decided_at, version, created_at",
        )
        .eq("work_product_id", data.id)
      ...
      context.supabase
        ...
        .select(
          "id, format, role, storage_ref, mime_type, size_bytes, version, generated_by, created_at",
        )
        .eq("work_product_id", data.id)
        .order("created_at", { ascending: false }),
    ...
    const { data: profiles } = people.length
      ...
      : { data: [] as any[] };
    const pmap = new Map(
      ((profiles ?? []) as any[]).map((p) => [p.id, p.display_name ?? p.email ?? p.id]),
    );
    ...
    return {
      ...
      versions: (versions.data ?? []).map((v: any) => ({
        ...v,
        authorName: pmap.get(v.author_id) ?? null,
      })),
      comments: (comments.data ?? []).map((c: any) => ({
        ...c,
        authorName: pmap.get(c.author_id) ?? null,
      })),
...
export const deleteWorkDeliverable = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) =>
    z.object({ ...commandMetadataSchema.shape, id: z.string().uuid() }).parse(i),
  )
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase
      .from("work_products")
...
export const getWorkDeliverableVersion = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) =>
    z.object({ id: z.string().uuid(), version: z.number().int().positive() }).parse(i),
  )
  .handler(async ({ data, context }) => {
  ...
  .handler(async ({ data, context }) => {
    ...
    const contextBlock = resolved.length
      ? resolved
          .map(
            (s) =>
              `- [${s.type}] ${s.title}${s.stamp ? ` (cập nhật ${s.stamp})` : ""}: ${s.snippet}`,
          )
          .join("\n")
...
export const getWorkDeliverableWeeklyReport = createServerFn({ method: "GET" })
  ...
  .handler(
    async ({ data, context }): Promise<WeeklyReport> =>
      computeWeeklyReport(context.supabase, data.workspaceId ?? null, data.days),
  );
...
async function computeWeeklyReport(
  ...
  const context = { supabase };
  {
    ...
    for (const p of (products ?? []) as any[])
      typeById.set(p.id as string, (p.business_type as string) ?? "OTHER");
    ...
    const rows = [...acc.values()].sort(
      (a, b) => order.indexOf(a.businessType) - order.indexOf(b.businessType),
    );
...
export const getWorkDeliverableArtifactUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!artifact?.storage_ref)
      throw new ApiError({ code: "RESOURCE_NOT_FOUND", message: "ARTIFACT_NOT_FOUND" });
...
export const listWorkProductShares = createServerFn({ method: "GET" })
  ...
  .inputValidator((i: unknown) => z.object({ id: z.string().uuid() }).parse(i))
  .handler(
    async ({ data, context }): Promise<{ shares: WorkProductShare[]; canManage: boolean }> => {
      const { data: rows, error } = await context.supabase
        .from("work_product_shares")
        .select(
          "id, workspace_id, shared_with_user_id, permission, note, status, expires_at, created_at",
        )
        .eq("work_product_id", data.id)
        .order("created_at", { ascending: true });
      if (error) mapPgError(error);
      ...
      const wsIds = [
        ...new Set(list.map((r) => r.workspace_id as string | null).filter(Boolean)),
      ] as string[];
      const userIds = [
        ...new Set(list.map((r) => r.shared_with_user_id as string | null).filter(Boolean)),
      ] as string[];
      ...
      if (userIds.length) {
        ...
        for (const u of (us ?? []) as any[])
          names.set(
            u.id as string,
            (u.display_name as string) ?? (u.primary_email as string) ?? "—",
          );
      }
      const now = Date.now();
      const { data: canEdit } = await context.supabase.rpc("can_edit_work_product", {
        _id: data.id,
      } as never);
      return {
        shares: list.map((r) => {
