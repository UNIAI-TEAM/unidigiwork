...
export const Route = createFileRoute("/_authenticated/m/admin/departments")({
  head: () => ({
    meta: [
      { title: "Quản lý bộ phận mobile — UNIWORK" },
      {
        name: "description",
        content: "Quản lý bộ phận và phân công thành viên theo tổ chức trên PWA UNIWORK.",
      },
      { property: "og:title", content: "Quản lý bộ phận mobile — UNIWORK" },
      {
...
function MobileDepartments() {
  ...
  const save = useMutation({
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["m-admin-departments"] });
      setSelected(null);
  ...
  const groups = useMemo(() => {
    ...
    for (const person of query.data?.people ?? []) {
      const key = person.department || t("m.admin.departmentNone");
  ...
  return (
    ...
    >
      ...
      {query.isLoading ? (
        ...
        <MobileAdminMessage retry={() => void query.refetch()}>
          {t("m.admin.departmentError")}
        </MobileAdminMessage>
      ) : null}
          ...
          <section key={name}>
            <div className="mb-2 flex items-center gap-2">
              ...
              <Badge variant="outline">
                {t("m.admin.departmentMembers").replace("{count}", String(people.length))}
              </Badge>
            </div>
            <div className="grid gap-2">
              {people.map((person) => (
                <button
                  ...
                  className="flex min-h-14 w-full items-center gap-3 rounded-xl border border-border p-3 text-left disabled:opacity-70"
                >
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("m.admin.departmentAssign")}</DialogTitle>
            <DialogDescription>{selected?.name}</DialogDescription>
          </DialogHeader>
          <div className="grid gap-2">
            <Label htmlFor="department">{t("m.admin.departmentName")}</Label>
            <Input
              id="department"
              value={department}
            ...
            <datalist id="department-list">
              {query.data?.departments.map((value) => (
                <option key={value} value={value} />
              ))}
            </datalist>
          </div>
          <DialogFooter>
            <Button variant="outline" className="min-h-11" onClick={() => setSelected(null)}>
              {t("m.admin.cancel")}
            </Button>
            <Button
              className="min-h-11"
