  ...
  head: () => ({
    meta: [
      { title: "Soạn kết quả công việc — UNIWORK" },
      {
        name: "description",
        content:
          "Soạn thảo cùng nhân sự AI với nguồn ngữ cảnh chọn lọc, phiên bản bất biến và quy trình duyệt.",
      },
...
function WorkProductDetail() {
  ...
  const [aiProvenance, setAiProvenance] = useState<
    Array<{ type: string; id: string; title: string; stamp: string | null }>
  >([]);
  ...
  useEffect(() => {
    ...
    setEnabled((prev) => {
      ...
      for (const s of sources) {
        const k = `${s.type}:${s.id}`;
        if (next[k] === undefined)
          next[k] = s.type === "WORKSPACE" || s.type === "MEETING_ARTIFACT";
      }
      return next;
    });
  }, [sources]);
  ...
  const save = useMutation({
    mutationFn: () =>
      updateWorkDeliverable({ data: { id, title: title.trim() || undefined, content } }),
    onSuccess: () => {
      setDirty(false);
  ...
  const applyAi = (mode: "insert" | "replace") => {
    ...
    setAiApplied(true);
    setAiProvenance(
      activeSources.map((s) => ({ type: s.type, id: s.id, title: s.title, stamp: s.stamp })),
    );
    setAiOut("");
  };
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                {dirty ? t("wp.save") : t("wp.saved")} · v{product.current_version}
              </p>
            </div>
            <Button
              variant="ghost"
            ...
            <Button
              ...
              disabled={!canEdit || snapshot.isPending}
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          ...
          <section className="flex min-w-0 flex-1 flex-col bg-surface">
            <div className="flex min-h-12 items-center gap-1 overflow-x-auto border-b bg-card px-3">
                ...
                onValueChange={(value) =>
                  changeWorkDeliverableStatus({
                    data: {
                      idempotencyKey: crypto.randomUUID(),
                      id,
                      status: value as (typeof WP_STATUSES)[number],
                    },
                  })
                    .then(invalidate)
              ...
              >
                <SelectTrigger className="mr-2 h-8 w-[145px] border-0 bg-surface-2 shadow-none">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {WP_STATUSES.map((status) => (
                    <SelectItem key={status} value={status}>
                      {t(`wp.status.${status}` as never)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              ...
              <Button
                ...
                aria-label={t("wp.editor.underline")}
              >
                <Underline />
              </Button>
              ...
              <Button
                ...
                aria-label={t("wp.editor.numbered")}
              >
                <ListOrdered />
              </Button>
              <Button
              ...
              <Button
                ...
                size="icon-sm"
                onClick={() => insertBlock("---")}
                aria-label={t("wp.editor.pagebreak")}
              >
                <Minus />
              </Button>
              <Button
              ...
              >
                ...
                <span className="hidden sm:inline">
                  {previewMode ? t("wp.editor.edit") : t("wp.editor.preview")}
                </span>
              </Button>
              <div className="ml-auto flex items-center gap-2 pr-2 text-xs text-muted-foreground">
                ...
                {data.workspace?.name && (
                  <span className="hidden xl:inline">{data.workspace.name}</span>
                )}
              </div>
            </div>
            {!canEdit && (
              <p className="border-b px-5 py-2 text-xs text-muted-foreground">{t("wp.readonly")}</p>
            )}
            <ScrollArea className="min-h-0 flex-1">
              {previewMode ? (
                ...
                <div className="mx-auto my-5 min-h-[calc(100vh-13rem)] w-[calc(100%-2rem)] max-w-[820px] border bg-card shadow-sm sm:my-8 sm:w-[calc(100%-4rem)]">
                  <div className="px-6 py-8 sm:px-12 sm:py-12 lg:px-16">
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-2 border-b pb-5 text-xs text-muted-foreground">
                      <span>{t(`wp.status.${product.status}` as never)}</span>
                      <span>·</span>
                      <span>v{product.current_version}</span>
                      <span>·</span>
                      <span>{fmt.format(new Date(product.updated_at))}</span>
                    </div>
                    <Textarea
                      ref={editorRef}
                      value={content}
            ...
            <aside className="fixed inset-x-0 bottom-0 top-28 z-40 w-full shrink-0 border-l bg-card shadow-md sm:left-auto sm:w-[360px] xl:static xl:z-auto xl:shadow-none">
              <Tabs defaultValue="ai" className="flex h-full flex-col">
                <div className="border-b px-4 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-semibold">{t("wp.tab.ai")}</p>
                      <p className="text-xs text-muted-foreground">UNI Assistant</p>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon-sm"
                ...
                >
                  ...
                  <TabsTrigger value="context" aria-label={t("wp.tab.context")}>
                    <Layers />
                  </TabsTrigger>
                  <TabsTrigger value="versions" aria-label={t("wp.tab.versions")}>
                    <History />
                  </TabsTrigger>
                  <TabsTrigger value="comments" aria-label={t("wp.tab.comments")}>
                    <MessageSquare />
                  </TabsTrigger>
                  <TabsTrigger value="review" aria-label={t("wp.tab.review")}>
                    <UserCheck />
                  </TabsTrigger>
                  <TabsTrigger value="links" aria-label={t("wp.tab.links")}>
                    <Link2 />
                  </TabsTrigger>
                  <TabsTrigger value="files" aria-label={t("wp.files.tab")}>
                    <Download />
                  </TabsTrigger>
                  <TabsTrigger value="share" aria-label={t("wp.share.tab")}>
                    <Share2 />
                  </TabsTrigger>
                  {isImportedDocx && (
                    <TabsTrigger value="docx" aria-label="Tài liệu Word">
                      <FileType2 />
                    </TabsTrigger>
                  )}
                </TabsList>
                ...
                <ScrollArea className="min-h-0 flex-1 px-4 pb-5">
                    ...
                    >
                      ...
                      <PromptInputFooter className="justify-between">
                        <span className="text-[11px] text-muted-foreground">
                          {activeSources.length} {t("wp.tab.context")}
                        </span>
                        <PromptInputSubmit
                          status={runAi.isPending ? "submitted" : undefined}
                          disabled={runAi.isPending}
                        />
                      </PromptInputFooter>
                    </PromptInput>
                    {runAi.isPending && (
                      <Shimmer className="text-sm">{`${t("wp.ai.run")}...`}</Shimmer>
                    )}
                    {aiOut && (
                      <div className="border-t pt-4">
                        ...
                        <Message from="assistant">
                          <MessageContent>
                            <MessageResponse>{aiOut}</MessageResponse>
                          </MessageContent>
                        </Message>
                        <div className="mt-3 flex flex-wrap gap-2">
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => navigator.clipboard.writeText(aiOut)}
                          >
                            {t("wp.ai.copy")}
                          </Button>
                        </div>
                  ...
                  <TabsContent value="context" className="mt-0 space-y-3">
                    ...
                    {(sources ?? []).length === 0 && (
                      <p className="text-sm text-muted-foreground">{t("wp.context.empty")}</p>
                    )}
                    {(sources ?? []).map((s) => {
                      const k = `${s.type}:${s.id}`;
                      return (
                  ...
                  <TabsContent value="versions" className="mt-0 space-y-2">
                    ...
                    {data.versions.map((v: any) => (
                      <div key={v.id} className="rounded-lg border bg-background p-3">
                        ...
                        {v.authorName && (
                          <p className="mt-1 text-xs text-muted-foreground">{v.authorName}</p>
                        )}
                        {Array.isArray(v.provenance) && v.provenance.length > 0 && (
                          <div className="mt-2">
                            <p className="text-[11px] font-medium text-muted-foreground">
                              {t("wp.versions.provenance")}
                            </p>
                  ...
                  <TabsContent value="comments" className="mt-0 space-y-3">
                    ...
                    {data.comments.length === 0 && (
                      <p className="text-sm text-muted-foreground">{t("wp.comments.empty")}</p>
                    )}
                  ...
                  <TabsContent value="review" className="mt-0 space-y-3">
                          ...
                          <SelectContent>
                            {(reviewers ?? []).map((r) => (
                              <SelectItem key={r.id} value={r.id}>
                                {(r as { self?: boolean }).self
                                  ? `${r.name} ${t("wp.review.self")}`
                                  : r.name}
                              </SelectItem>
                            ))}
                          </SelectContent>
                    ...
                    {data.reviews.map((r: any) => (
                      <div key={r.id} className="rounded-lg border bg-background p-3">
                        ...
                        {r.decision_note && (
                          <p className="mt-1 text-xs text-muted-foreground">{r.decision_note}</p>
                        )}
                        {r.status === "PENDING" && (
                  ...
                  <TabsContent value="files" className="mt-0 space-y-3">
                    <div>
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-2">
                        {(["DOCX", "XLSX", "PPTX", "PDF"] as const).map((f) => (
                          <Button
                            ...
                            className="gap-2"
                          >
                            {exportArtifact.isPending && exportingFormat === f ? (
                      ...
                      <Button
                        ...
                        disabled={!canEdit || exportArtifact.isPending}
                        onClick={() =>
                          exportArtifact.mutate({ format: "DOCX", engine: "GENOFFICE" })
                        }
                      >
                      ...
                      <div className="space-y-2 rounded-lg border bg-background p-3">
                        ...
                        <Select value={shareTarget} onValueChange={setShareTarget}>
                          ...
                          <SelectContent>
                            {shareOptions
                              .filter(
                                (w) => !(shareData?.shares ?? []).some((s) => s.targetId === w.id),
                              )
                              .map((w) => (
                        ...
                        <Select
                          value={sharePerm}
                          onValueChange={(v) => setSharePerm(v as "VIEW" | "EDIT")}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <div key={s.id} className="space-y-2 rounded-lg border bg-background p-3">
                        <div className="flex items-center gap-2">
                          <Badge variant="secondary" className="text-[10px]">
                            {s.targetType === "USER"
                              ? t("wp.share.kindUser")
                              : t("wp.share.kindWorkspace")}
                          </Badge>
                          <span className="min-w-0 flex-1 truncate text-sm">{s.targetName}</span>
                          <Badge
                            variant={
                              s.status === "REVOKED" || s.expired ? "destructive" : "outline"
                            }
                            className="text-[10px]"
                          >
                            {s.status === "REVOKED"
                        ...
                        {shareData?.canManage && (
                          <div className="flex flex-wrap gap-2">
                            <Button
                              ...
                              onClick={() =>
                                patchShare(s.id, {
                                  permission: s.permission === "EDIT" ? "VIEW" : "EDIT",
                                })
                              }
                            >
                            ...
                            <Button
                              ...
                              onClick={() =>
                                patchShare(s.id, {
                                  status: s.status === "REVOKED" ? "ACTIVE" : "REVOKED",
                                })
                              }
                            >
                              {s.status === "REVOKED"
                            ...
                            {s.expiresAt && (
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => patchShare(s.id, { expiresAt: null })}
                              >
