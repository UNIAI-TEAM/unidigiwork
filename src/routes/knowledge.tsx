...
export const Route = createFileRoute("/knowledge")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Trung tâm tri thức nội bộ với AI Copilot, SOP, Playbooks, Wiki.",
      },
    ],
  }),
});
...
const featured = [
  {
    title: "Quy trình release Sprint 6",
    type: "SOP",
    icon: FileText,
...
const categories = [
  {
    name: "SOP Library",
    count: 86,
    desc: "Quy trình và hướng dẫn chuẩn cho công việc",
    icon: FileText,
  ...
  },
  {
    name: "FAQ",
    count: 54,
    desc: "Câu hỏi thường gặp và giải đáp",
    icon: HelpCircle,
  ...
  },
  {
    name: "Technical Wiki",
    count: 112,
    desc: "Tài liệu kỹ thuật, kiến trúc, API, tích hợp",
    icon: Code2,
  ...
  },
  {
    ...
    count: 32,
    desc: "Bài học kinh nghiệm từ các dự án",
    icon: Lightbulb,
  ...
  },
  {
    ...
    count: 26,
    desc: "Tóm tắt và insight từ các cuộc họp",
    icon: Video,
  ...
  },
  {
    ...
    count: 18,
    desc: "Chính sách, quy định và tuân thủ",
    icon: ShieldCheck,
  ...
  },
  {
    name: "Playbooks",
    count: 24,
    desc: "Hướng dẫn triển khai và best practices",
    icon: PlayCircle,
  ...
  },
  {
    name: "Knowledge Maps",
    count: 14,
    desc: "Bản đồ tri thức và mối liên kết",
    icon: Network,
...
const recent = [
  ...
  {
    title: "Mobile App offline sync — Lessons",
    type: "Lessons",
    team: "Mobile Team",
    time: "Yesterday",
    author: "Lê Văn C",
          ...
          <aside className="hidden w-64 shrink-0 flex-col border-r border-border bg-surface md:flex">
            ...
            <nav className="mt-2 flex-1 overflow-y-auto px-3 pb-4">
              {spaces.map((s) => (
                <button
                  key={s.name}
                  className={`flex w-full items-center justify-between rounded-lg px-3 py-1.5 text-sm ${s.active ? "bg-primary/15 text-foreground" : "text-muted-foreground hover:bg-surface-2 hover:text-foreground"}`}
                >
                  <span>{s.name}</span>
                  {s.count !== null && (
              ...
              <div className="space-y-1.5 px-2">
                {tags.map((t) => (
                  <button
                    key={t.name}
                    className="flex w-full items-center justify-between rounded-lg px-2 py-1 text-sm hover:bg-surface-2"
                  >
                    <span className={`rounded px-2 py-0.5 text-[11px] font-medium ${t.color}`}>
                      {t.name}
                    </span>
          ...
          <main className="min-w-0 flex-1 overflow-y-auto">
            <div className="mx-auto max-w-5xl space-y-6 px-4 py-6 sm:px-6">
              ...
              <div className="rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/10 via-violet-500/5 to-transparent p-6">
                <h2 className="text-center text-base font-medium text-foreground/90">
                  Ask anything. Get answers from your organization.
                </h2>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  {featured.map((c) => (
                    <article
                      key={c.title}
                      className="group rounded-xl border border-border bg-surface p-4 hover:border-primary/40"
                    >
                      <div
                      ...
                      <span
                        className={`mt-2 inline-block rounded px-1.5 py-0.5 text-[10px] font-medium ${c.color}`}
                      >
                        {c.type}
                      </span>
              ...
              <section>
                ...
                <div className="overflow-hidden rounded-xl border border-border bg-surface">
                  {recent.map((r, i) => (
                    <div
                      key={r.title}
                      className={`flex items-center gap-3 px-4 py-3 hover:bg-surface-2 ${i > 0 ? "border-t border-border" : ""}`}
                    >
                      <span
                        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${r.color}`}
                      >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`shrink-0 rounded px-1.5 py-0.5 text-[10px] font-medium ${r.color}`}
                          >
                            {r.type}
                          </span>
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {r.team} · Updated {r.time}
                        </div>
                      </div>
                      <img
                        src={avatar(r.author)}
            ...
            <aside className="hidden w-80 shrink-0 flex-col gap-4 overflow-y-auto border-l border-border bg-surface px-4 py-5 xl:flex">
              ...
              <div className="rounded-xl border border-border bg-surface-2/50 p-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Quy trình release Sprint 6 bao gồm 6 bước chính:
                </p>
                ...
                <div className="mt-1 flex gap-1">
                  {[FileText, FileVideo, FileCode, FileImage, FileSpreadsheet].map((Icon, i) => (
                    <span
                      key={i}
                      className="flex h-6 w-6 items-center justify-center rounded bg-surface text-muted-foreground"
                    >
                      <Icon className="h-3 w-3" />
                    </span>
                  ))}
                </div>
                <button className="mt-3 w-full rounded-lg border border-border bg-surface py-2 text-xs hover:bg-surface-2">
                  View full answer
                </button>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {related.map((r) => (
                    <button
                      key={r.title}
                      className="flex w-full items-start gap-2 rounded-lg p-2 text-left hover:bg-surface-2"
                    >
                      <FileText className="mt-0.5 h-4 w-4 shrink-0 text-primary" />
                      <div className="min-w-0">
                        <div className="truncate text-xs font-medium">{r.title}</div>
                        <div className="text-[10px] text-muted-foreground">
                          {r.type} · Updated {r.date}
                        </div>
                      </div>
                    </button>
                  ))}
              ...
              <div>
                ...
                <button className="mt-2 w-full rounded-lg border border-border bg-surface py-2 text-xs hover:bg-surface-2">
                  Create articles from gaps
                </button>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {popular.map((p) => (
                    <button
                      key={p.q}
                      className="flex w-full items-center justify-between gap-2 rounded-lg p-2 text-left hover:bg-surface-2"
                    >
