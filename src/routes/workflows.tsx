...
const workflows: WF[] = [
  {
    ...
    subtitle: "HR & Admin",
    category: "HR & Admin",
    catColor: "bg-violet-500/20 text-violet-300 border border-violet-500/30",
    icon: UsersIcon,
  ...
  },
  {
    ...
    subtitle: "IT & Ops",
    category: "IT & Ops",
    catColor: "bg-sky-500/20 text-sky-300 border border-sky-500/30",
    icon: Monitor,
  ...
  },
  {
    ...
    subtitle: "Finance",
    category: "Finance",
    catColor: "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30",
    icon: DollarSign,
  ...
  },
  {
    ...
    catColor: "bg-amber-500/20 text-amber-300 border border-amber-500/30",
    icon: FolderKanban,
  ...
  },
  {
    ...
    catColor: "bg-rose-500/20 text-rose-300 border border-rose-500/30",
    icon: ShieldCheck,
...
const categories = [
  ...
  {
    name: "IT & Ops",
    desc: "IT Request, Access, Changes…",
    count: 8,
    icon: Monitor,
  ...
  },
  {
    ...
    desc: "Project Intake, Change Request…",
    count: 6,
    icon: FolderKanban,
          ...
          <main className="min-w-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6">
            ...
            <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
              {categories.map((c) => (
                <button
                  key={c.name}
                  className="flex flex-col items-start gap-2 rounded-xl border border-border bg-surface p-4 text-left transition-colors hover:border-primary/40"
                >
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2">
              ...
              <Select
                value={cat}
                onChange={setCat}
                label={t("wf.filter.category")}
                options={["All", ...cats]}
              />
              <Select
                value={owner}
                onChange={setOwner}
                label={t("wf.filter.owner")}
                options={["All", ...owners]}
              />
            ...
            <div className="mt-3 overflow-hidden rounded-xl border border-border bg-surface">
                      ...
                      >
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div
                              className={`flex h-9 w-9 items-center justify-center rounded-lg ${w.iconBg}`}
                            >
                        ...
                        <td className="px-3 py-3">
                          <span
                            className={`rounded-md px-2 py-0.5 text-[11px] font-medium ${w.catColor}`}
                          >
                            {w.category}
                          </span>
                        </td>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3 text-xs text-muted-foreground">
                <div>
                  {t("wf.showing")} 1 {t("wf.of")} {filtered.length} {t("wf.of")} 42{" "}
                  {t("wf.cat.workflows")}
                </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        ...
        <dl className="mt-4 space-y-2.5 text-sm">
          ...
          <Row label={t("wf.panel.category")}>
            <span className={`rounded-md px-2 py-0.5 text-[11px] font-medium ${wf.catColor}`}>
              {wf.category}
            </span>
          </Row>
          <Row label={t("wf.panel.created")}>
            <span className="text-muted-foreground">10/03/2025 10:30 AM</span>
          </Row>
          <Row label={t("wf.panel.updated")}>
            <span className="text-muted-foreground">{wf.updated} 09:15 AM</span>
          </Row>
          <Row label={t("wf.panel.avg")}>
            <span className="text-muted-foreground">{wf.avgTime}</span>
          </Row>
        </dl>
...
function Node({
  ...
  return (
    <div
      className={`rounded-md border px-3 py-1.5 font-medium ${tones[tone]} ${small ? "text-[10px]" : ""}`}
    >
      {label}
    </div>
  );
