...
function Landing() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) { toast.error(t("land.email") + " / " + t("land.password")); return; }
    setLoading(true);
      ...
      <header className="sticky top-0 z-40 border-b border-border/60 bg-background/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <nav className="hidden items-center gap-6 text-sm text-muted-foreground md:flex">
            <a href="#features" className="hover:text-foreground">{t("land.nav.features")}</a>
            <a href="#preview" className="hover:text-foreground">{t("land.nav.preview")}</a>
            <a href="#login" className="hover:text-foreground">{t("land.nav.login")}</a>
            <Link to="/meeting" className="hover:text-foreground">{t("land.nav.demo")}</Link>
          </nav>
          <div className="flex items-center gap-2">
            <LanguageToggle />
            <ThemeToggle />
            <Link to="/auth" className="hidden rounded-lg px-3 py-2 text-sm text-muted-foreground hover:text-foreground sm:inline">{t("land.nav.signup")}</Link>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:py-24">
          <div className="flex flex-col justify-center">
            ...
            <h1 className="text-4xl font-bold leading-tight tracking-tight sm:text-5xl lg:text-6xl">
              {t("land.h1.a")}<br />
              <span className="bg-gradient-to-r from-primary to-violet-400 bg-clip-text text-transparent">{t("land.h1.b")}</span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-muted-foreground">
              <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-success" /> {t("land.bullet.security")}</span>
              <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-success" /> {t("land.bullet.vi")}</span>
              <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-success" /> {t("land.bullet.deploy")}</span>
            </div>
          </div>
          ...
          <div id="login" className="flex items-center justify-center">
            <div className="w-full max-w-md rounded-2xl border border-border bg-surface/80 p-6 shadow-2xl shadow-primary/5 backdrop-blur">
              ...
              <form onSubmit={handleLogin} className="mt-5 space-y-3">
                <div>
                  <label className="text-xs font-medium text-muted-foreground">{t("land.email")}</label>
                ...
                <div>
                  <label className="text-xs font-medium text-muted-foreground">{t("land.password")}</label>
              ...
              <div className="mt-4 text-center text-xs text-muted-foreground">
                {t("land.no.account")} <Link to="/auth" className="text-primary hover:underline">{t("land.create.account")}</Link>
              </div>
            </div>
          </div>
        </div>
