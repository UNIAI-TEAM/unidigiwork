...
export function MobileTaskDetail({ id }: { id: string }) {
  ...
  const members = useQuery({
    ...
    enabled: Boolean(task?.workspace_id),
  });
  const invalidate = async () =>
    Promise.all([
      qc.invalidateQueries({ queryKey: ["task-detail", id] }),
      qc.invalidateQueries({ queryKey: ["task-ops-board"] }),
      qc.invalidateQueries({ queryKey: ["m-work-graph"] }),
    ]);
  const notifyError = (error: unknown) =>
    toast.error(error instanceof Error ? error.message : t("m.tasks.actionError"));
  ...
  const editMutation = useMutation({
    mutationFn: (patch: { title?: string; description?: string; priority?: Priority }) =>
      updateTask({
        data: {
          taskId: id,
          ...patch,
  ...
  const assignMutation = useMutation({
    ...
    onSuccess: async () => {
      await invalidate();
      toast.success(t("m.tasks.assigned"));
    },
    onError: notifyError,
  });
  ...
  const removeAttachment = useMutation({
    mutationFn: async ({
      ...
      await deleteTaskAttachment({ data: { attachmentId } });
      await removeTaskAttachmentObject(storagePath);
    },
    onSuccess: invalidate,
    onError: notifyError,
  });
  ...
  if (detail.isError || !task)
    return (
      <main className="p-4">
        <p className="text-sm text-muted-foreground">{t("m.tasks.notFound")}</p>
        <Button
          variant="outline"
  ...
  const nameFor = (userId: string) =>
    members.data?.find((member) => member.userId === userId)?.name ?? userId.slice(0, 8);
  const localDue = task.due_at
  ...
  const share = async () => {
    ...
    try {
      if (navigator.share) await navigator.share({ title: task.title, url });
        ...
        toast.success(t("m.tasks.linkCopied"));
      }
    } catch {
      /* share dismissed */
    }
    ...
    <main className="mx-auto flex min-h-full w-full max-w-3xl flex-col overflow-x-hidden pb-24">
      <header className="sticky top-0 z-20 flex min-w-0 items-center gap-2 border-b border-border bg-background/95 px-3 py-2 backdrop-blur">
        ...
        <Button
          ...
          onClick={() => followMutation.mutate(!follow.data?.following)}
          aria-label={t("m.tasks.follow")}
        >
          {follow.data?.following ? <BellOff className="h-5 w-5" /> : <Bell className="h-5 w-5" />}
        </Button>
        <Button
          variant="ghost"
          size="icon"
      ...
      <div className="grid gap-4 px-4 pt-4">
        <section className="border-b border-border pb-4">
          <div className="flex flex-wrap gap-2">
            <Badge variant="secondary">{t(`m.tasks.status.${task.status}` as never)}</Badge>
            <Badge variant="outline">{t(`m.tasks.priority.${task.priority}` as never)}</Badge>
            {overdue ? <Badge variant="destructive">{t("m.tasks.due.overdue")}</Badge> : null}
          </div>
          <div className="mt-4 flex items-center justify-between text-sm">
            <span className="font-medium">{t("m.tasks.progress")}</span>
            <span className="text-muted-foreground">
              {subtasks.length ? `${done}/${subtasks.length}` : `${progress}%`}
            </span>
          </div>
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-surface-2">
            <div className="h-full rounded-full bg-primary" style={{ width: `${progress}%` }} />
          </div>
          <Button className="mt-4 min-h-11 w-full" onClick={() => setEditOpen(true)}>
        ...
        <Tabs defaultValue="overview" className="min-w-0">
          <TabsList className="-mx-4 flex h-12 w-auto justify-start gap-1 overflow-x-auto rounded-none border-b border-border bg-transparent px-4">
            {["overview", "chat", "subtasks", "files", "related"].map((tab) => (
              <TabsTrigger
                key={tab}
                value={tab}
                className="min-h-11 shrink-0 rounded-lg px-3 data-[state=active]:bg-muted"
              >
                {t(`m.tasks.tab.${tab}` as never)}
              </TabsTrigger>
            ))}
          </TabsList>
          ...
          <TabsContent value="overview" className="mt-4 grid gap-5">
            <section>
              <h2 className="text-sm font-semibold">{t("m.tasks.description")}</h2>
              <p className="mt-2 whitespace-pre-wrap break-words text-sm leading-6 text-muted-foreground">
                {task.description || t("m.tasks.noDescription")}
              </p>
            </section>
            <section className="grid gap-3 border-y border-border py-4">
              ...
              <label className="grid gap-1.5 text-xs font-medium text-muted-foreground">
                ...
                <select
                  ...
                  disabled={assignMutation.isPending || members.isLoading}
                  onChange={(event) =>
                    event.target.value && assignMutation.mutate(event.target.value)
                  }
                >
                  <option value="">
                    {assignees.length
                      ? assignees.map((item) => nameFor(item.user_id)).join(", ")
                      : t("tops.unassigned")}
                  </option>
                  {(members.data ?? [])
                    .filter((member) => !assignees.some((item) => item.user_id === member.userId))
                    .map((member) => (
            ...
            <section>
              <h2 className="text-sm font-semibold">{t("m.tasks.tags")}</h2>
              <div className="mt-2 flex flex-wrap gap-2">
                {(task.tags ?? []).map((tag) => (
                  <Badge key={tag} variant="outline" className="min-h-8 gap-1 pl-3">
                    {tag}
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-7 w-7 rounded-full"
                      aria-label={t("m.tasks.removeTag")}
                      onClick={() =>
                        tagsMutation.mutate((task.tags ?? []).filter((value) => value !== tag))
                      }
                    >
                      <X className="h-3 w-3" />
                    </Button>
              ...
              <form
                ...
                onSubmit={(event) => {
                  ...
                  if (next && !(task.tags ?? []).includes(next))
                    tagsMutation.mutate([...(task.tags ?? []), next]);
                  setTagDraft("");
                }}
              >
                <Input
          ...
          <TabsContent value="subtasks" className="mt-4 grid gap-3">
            {subtasks.length ? (
              subtasks.map((subtask) => (
                <div
                  key={subtask.id}
                  className="flex min-h-14 items-center gap-2 border-b border-border py-2"
                >
                  <Button
                    variant="ghost"
                    size="icon"
                    ...
                    onClick={() =>
                      statusMutation.mutate({
                        ...
                        rowVersion: subtask.row_version,
                      })
                    }
                    aria-label={t("m.tasks.toggleSubtask")}
                  >
                    <CheckCircle2
                      className={
                        subtask.status === "done"
                  ...
                  <Link
                    ...
                    className={`min-w-0 flex-1 break-words text-sm ${subtask.status === "done" ? "text-muted-foreground line-through" : ""}`}
                  >
                    {subtask.title}
                  </Link>
                </div>
            ...
            <form
              ...
              onSubmit={(event) => {
                event.preventDefault();
                if (subtaskTitle.trim()) subtaskMutation.mutate(subtaskTitle.trim());
              }}
            >
              <Input
                value={subtaskTitle}
                onChange={(event) => setSubtaskTitle(event.target.value)}
                placeholder={t("m.tasks.addSubtask")}
                className="h-11 min-w-0"
              />
              <Button
                type="submit"
            ...
            <section className="mt-3 border-t border-border pt-4">
              <h2 className="text-sm font-semibold">
                {t("m.tasks.comments").replace("{n}", String(comments.length))}
              </h2>
              <div className="mt-3 grid max-h-80 gap-4 overflow-y-auto">
                {comments.length ? (
                  comments.map((item) => (
                    <article key={item.id}>
                      <p className="text-xs text-muted-foreground">
                        <span className="font-medium text-foreground">
                          {item.author_name ?? t("m.taskChat.member")}
                        </span>{" "}
                        ·{" "}
                  ...
                  <p className="text-sm text-muted-foreground">{t("m.tasks.noComments")}</p>
                )}
              </div>
              <form
              ...
              >
                <Textarea
                  ...
                  placeholder={t("m.tasks.addComment")}
                  className="min-w-0"
                  rows={2}
                />
                <Button
                  ...
                  aria-label={t("m.tasks.sendComment")}
                >
          ...
          <TabsContent value="files" className="mt-4 grid gap-2">
            {attachments.length ? (
              attachments.map((attachment) => (
                <div
                  key={attachment.id}
                  className="flex min-h-14 min-w-0 items-center gap-2 border-b border-border py-2"
                >
                  ...
                  <Button
                    ...
                    size="icon"
                    className="h-11 w-11 shrink-0"
                    aria-label={t("m.tasks.downloadFile")}
                    onClick={async () =>
                      window.open(
                        await getTaskAttachmentUrl(attachment.storage_path, true),
                        "_blank",
                      )
                    }
                  >
                  ...
                  <Button
                    ...
                    aria-label={t("m.tasks.deleteFile")}
                    onClick={() =>
          ...
          <TabsContent value="related" className="mt-4">
            <Button asChild variant="outline" className="mb-4 min-h-11 w-full">
              <Link to="/m/work-graph" search={{ task: id }}>
                {t("m.tasks.openGraph")}
              </Link>
            </Button>
            <RelatedWorkPanel
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="w-[calc(100vw-2rem)] max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle>{t("m.tasks.edit")}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-3">
            ...
            <label className="grid gap-1.5 text-xs font-medium text-muted-foreground">
              ...
              <select
                ...
                className="h-11 rounded-lg border border-border bg-background px-3 text-sm text-foreground"
                onChange={(event) =>
                  editMutation.mutate({ priority: event.target.value as Priority })
                }
              >
            ...
            <label className="grid gap-1.5 text-xs font-medium text-muted-foreground">
              ...
              >
                {["todo", "in_progress", "blocked", "done", "canceled"].map((value) => (
                  <option key={value} value={value}>
          ...
          <DialogFooter>
            <Button
              ...
              disabled={!titleDraft.trim() || editMutation.isPending}
              onClick={() =>
                editMutation.mutate({ title: titleDraft.trim(), description: descriptionDraft })
              }
            >
